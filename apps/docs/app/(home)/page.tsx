import { Cloud, Lock, MessageSquare, Radio, Smile, Users, Zap } from "lucide-react";
...
const features = [
	{
		...
		title: "Live RTMPS in",
		description:
			"Push from OBS to GetStream. Browser-side player + chat ship in two lazy-loaded chunks.",
	},
	{
		icon: Lock,
		title: "Invite-only",
		description: "One broadcaster. Magic-link invites. No public mode, no signups, no tier logic.",
	},
	{
		icon: MessageSquare,
		title: "Chat with emotes",
		description: "GetStream Chat with 7TV, BTTV, FFZ, and Twitch emotes merged via 12h cron.",
	},
	{
		icon: Cloud,
		title: "Cloudflare-native",
		description: "Two Workers (web + api), one D1, two R2 buckets, one KV. Free tier covers it.",
	},
];

const stats = [
	{ icon: Users, value: "1", label: "Broadcaster" },
	{ icon: Smile, value: "4", label: "Emote providers" },
	{ icon: Zap, value: "$0", label: "Cloudflare cost" },
];
			...
			<section className="relative flex flex-col items-center justify-center px-4 pt-32 pb-20 text-center">
				...
				<p className="mb-6 inline-flex items-center gap-2 rounded-full border border-fd-border bg-fd-card px-4 py-1.5 text-xs font-medium tracking-wide text-fd-muted-foreground uppercase">
					<span
						...
						style={{ background: "var(--color-fd-primary)" }}
					/>
					Self-hosted invite-only live streaming
				</p>
				...
				<p className="mx-auto mb-10 max-w-lg text-lg leading-relaxed text-fd-muted-foreground">
					HowlCast is a single-tenant invite-only live streaming platform. RTMPS via GetStream, chat
					via GetStream Chat, auth via Better Auth, hosted on two Cloudflare Workers.
				</p>
